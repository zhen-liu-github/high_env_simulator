...
class Road(object):
    ...
    def close_vehicles_to(self, vehicle: 'kinematics.Vehicle', distance: float, count: int = None,
        ...
        vehicles = [v for v in self.vehicles
                    ...
                    and (see_behind or - 100 * vehicle.LENGTH < vehicle.lane_distance_to(v))]

        vehicles = sorted(vehicles, key=lambda v: v.position[0])
        #print('vehicle size: {}, all size: {}'.format(len(vehicles), len(self.vehicles)))
        #print(distance)
        if count:
